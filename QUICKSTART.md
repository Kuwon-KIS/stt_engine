...
STT Engine은 각 음성 변환 작업의 성능 지표를 자동으로 수집하고 표시합니다:

#### 수집되는 지표:
- **CPU 사용률** - 평균(%) / 최대(%)
- **메모리 사용** - 평균(MB) / 최대(MB) 
- **GPU VRAM** - 현재 사용량(MB) / 최대 사용량(MB)
- **GPU 활용도** - 사용률(%)
- **처리 시간** - 총 소요 시간(초)

#### 웹 UI에서 확인:
1. **단일 파일 변환** - 결과 하단의 "📊 성능 메트릭" 섹션
2. **배치 처리** - 테이블의 "성능" 열 또는 "📊 성능 통계" 버튼 클릭
...
#### 성능 로그 저장:
- **위치**: `/app/data/{file_id}.performance.json`
- **배치 작업시**: 각 파일별 독립적인 성능 로그 생성
- **자동 저장**: 변환 완료 후 자동으로 저장됨
...
| 메모리 사용 (CPU) | ~2-3GB |
| 메모리 사용 (GPU) | ~8GB |
| CPU 활용도 | 70-90% (GPU 비사용 시) |
| GPU 활용도 | 85-95% (GPU 사용 시) |
