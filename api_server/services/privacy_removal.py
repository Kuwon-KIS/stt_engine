...
class QwenClient:
    """Qwen (Alibaba) LLM 클라이언트 (OpenAI 호환 API 사용)"""
    ...
    def _normalize_api_base(api_base: str) -> str:
        """
        base_url 정규화 (OpenAI SDK 호환)
        
        - /chat/completions 제거 (SDK가 자동으로 추가)
        - /v1로 끝남 (없으면 추가)
        """
...
class PrivacyRemovalService:
    ...
    def __init__(self, prompts_dir: Optional[str] = None):
        ...
        logger.info(f"PrivacyRemovalService 초기화: default_model={self.model_name}")
    ...
    async def initialize(self, model_name: Optional[str] = None):
        ...
        Args:
            model_name: 사용할 모델명 (None이면 환경변수에서 읽음)
        """
        # 사용할 모델 결정
...
_privacy_removal_service: Optional[PrivacyRemovalService] = None


def get_privacy_removal_service(prompts_dir: Optional[str] = None) -> PrivacyRemovalService:
    """
    ...
    """
    global _privacy_removal_service
    
    if _privacy_removal_service is None:
        logger.info("PrivacyRemovalService 싱글톤 생성")
        _privacy_removal_service = PrivacyRemovalService(prompts_dir)
    
    return _privacy_removal_service


async def _async_get_privacy_removal_service(prompts_dir: Optional[str] = None) -> PrivacyRemovalService:
    """
    ...
    Returns:
        PrivacyRemoverService 인스턴스
    """
    return get_privacy_removal_service(prompts_dir)
