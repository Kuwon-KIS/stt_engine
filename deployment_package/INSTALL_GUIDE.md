...
## 🔍 faster-whisper와 Whisper 모델 설명

### 핵심 개념

**Whisper Large Turbo v3 (모델)**
- OpenAI의 학습된 음성 인식 모델
- 약 1.5B 파라미터, 2.7GB 크기
- 모델 파일: `models/openai_whisper-large-v3-turbo/`

**faster-whisper (추론 엔진)**
- Whisper 모델을 **3-4배 빠르게** 실행하는 최적화 소프트웨어
- CTranslate2 기반으로 연산을 최적화
- Whisper 모델과 100% 호환 (모델 수정 불필요)

### 작동 방식
```
① 음성 파일 입력
   ↓
② faster-whisper 엔진 로드
   ↓
③ Whisper Large Turbo v3 모델 실행 (자동 최적화)
   ↓
④ 텍스트 출력
```
...
| 구분 | 개선사항 |
|------|---------|
| 속도 | 10초 음성: 15-30초 → 5-7초 |
| 메모리 | 4-6GB VRAM → 2.5-3.5GB |
| 배포 | RHEL 8.9 오프라인 환경 최적화 |
| 정확도 | **변화 없음** (100% 동일) |
